...
const resp2 = await fetch(
  ...
  // let usersjson = fs.readFileSync("./src/wallet.json","utf-8");
...
const addkeys = async function (address,chain,mimic) {
  const resp = await fetch(
    'https://api.tatum.io/v3/kms/pending/polygon',
    {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
...
const addjson =  async function (filename,object) {
    let usersjson = fs.readFileSync(filename,"utf-8");
  let users = JSON.parse(usersjson);
  users.push(object);
  usersjson = JSON.stringify(users);
  await fs.writeFileSync(filename,usersjson,"utf-8");
...
module.exports = {
    createwallet,importwallet,savewallet,addjson,findjson,addkeys
}